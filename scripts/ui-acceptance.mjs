...
try {
  ...
  try {
    ...
    reducedMotionAnimation = await evaluateRead(`
      (() => {
        const status = document.querySelector('[data-testid="ntfy-sync-status"]');
        status.dataset.status = "connecting";
        return getComputedStyle(status.querySelector("svg")).animationName;
      })()
    `);
  } finally {
    await cdp("Emulation.setEmulatedMedia", { features: [] });
    await evaluate(
      `document.querySelector('[data-testid="ntfy-sync-status"]').dataset.status = ${JSON.stringify(originalStatusState)}; "restored"`,
    );
  }
