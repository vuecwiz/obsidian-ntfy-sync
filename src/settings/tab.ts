...
export class NtfySyncSettingTab extends PluginSettingTab {
  ...
  override getSettingDefinitions(): SettingDefinitionItem[] {
    ...
    const connectionDefinitions: SettingDefinitionRender[] = [
      ...
      definition(t("settings.topics"), t("settings.topicsDesc"), (setting) => {
        setting
          .setClass("ntfy-sync-control-14rem")
          ...
          .addText((text) => {
            ...
            text.setValue(connection.topics.join(", ")).onChange(async (value) => {
              ...
              if (!topics.length || topics.some((topic) => !isValidNtfyTopic(topic))) {
                setInlineValidation(setting, t("settings.topicInvalid"), "ntfy-topics-validation");
                return;
              }
              ...
              clearInlineValidation(setting, t("settings.topicsDesc"));
        ...
        if (
          !connection.topics.length ||
          connection.topics.some((topic) => !isValidNtfyTopic(topic))
        ) {
          setInlineValidation(setting, t("settings.topicInvalid"), "ntfy-topics-validation");
        }
      }),
      definition(t("settings.mode"), undefined, (setting) => {
      ...
      definition(t("settings.authentication"), undefined, (setting) => {
        setting.addDropdown((dropdown) => {
          ...
          dropdown
            ...
            .onChange(async (kind) => {
              connection.readAuth = authForKind(kind);
              await this.plugin.saveSettings(true);
              this.refreshSettings();
            });
        });
      }),
      ...this.getAuthSettingDefinitions(connection.readAuth, false, definition),
      definition(t("settings.allowLoopback"), t("settings.allowLoopbackDesc"), (setting) => {
        setting.addToggle((toggle) =>
      ...
      definition(t("settings.publishResults"), t("settings.publishResultsDesc"), (setting) => {
        setting
          .setClass("ntfy-sync-control-14rem")
    ...
    if (connection.result) {
      ...
      connectionDefinitions.push(
          ...
          setting
            ...
            .addText((text) => {
              ...
              text.setValue(result.topic).onChange(async (value) => {
                ...
                clearInlineValidation(setting, t("settings.resultTopicDesc"));
                result.topic = topic;
                await this.plugin.saveSettings(false);
              });
            });
          resultTopicSetting = setting;
          refreshResultTopicValidation(
            setting,
            result.topic,
            connection.topics,
        ...
        definition(t("settings.resultPrivacy"), undefined, (setting) => {
          setting.addDropdown((dropdown) =>
            dropdown
              .addOptions({
        ...
        definition(t("settings.cacheResults"), undefined, (setting) => {
          setting.addToggle((toggle) =>
        ...
        definition(t("settings.resultAuthentication"), undefined, (setting) => {
          setting.addDropdown((dropdown) => {
            ...
            dropdown
              ...
              .onChange(async (kind) => {
                result.writeAuth = authForKind(kind);
                await this.plugin.saveSettings(true);
                this.refreshSettings();
              });
          });
        }),
        ...this.getAuthSettingDefinitions(result.writeAuth, true, definition),
      );
    }
  ...
  private getAuthSettingDefinitions(
    ...
    if (auth.kind === "basic") {
      return [
        definition(username, undefined, (setting) => {
        ...
        definition(passwordLabel, t("settings.credentialWarning"), (setting) => {
          setting.descEl.dataset.testid = `ntfy-${scope}-credential-warning`;
          setting.descEl.setAttr("role", "note");
          setting.addText((text) => {
    ...
    if (auth.kind === "bearer") {
      return [
        definition(tokenLabel, t("settings.credentialWarning"), (setting) => {
          setting.descEl.dataset.testid = `ntfy-${scope}-credential-warning`;
          setting.descEl.setAttr("role", "note");
          setting.addText((text) => {
            text.inputEl.type = "password";
  ...
  private getRuleListDefinition(definition: DefinitionFactory): SettingDefinitionItem {
    const t: I18n["t"] = (key, variables) => this.plugin.i18n.t(key, variables);
    const rules = rulesOf(this.plugin);
    const items: SettingDefinitionRender[] = rules.length
      ? rules.map((rule, index) => {
          const info = summarizeRule(rule, this.plugin.i18n);
          return definition(info.name, info.description, (setting) => {
            this.markDeclarativeRuleList(setting);
            this.configureRuleSetting(setting, rule, index);
          });
        })
      : [
          definition(t("settings.noRules"), undefined, (setting) => {
            this.markDeclarativeRuleList(setting);
            setting.settingEl.addClass("ntfy-sync-rule-empty");
            setting.settingEl.dataset.testid = "ntfy-rule-empty";
          }),
        ];

    return {
      type: "group",
      cls: "ntfy-sync-rule-list",
      items,
    };
  }

  private markDeclarativeRuleList(setting: Setting): void {
    const list = setting.settingEl.closest<HTMLElement>(".ntfy-sync-rule-list");
    if (list) list.dataset.testid = "ntfy-rule-list";
  }

  private decorateContainer(): void {
    this.containerEl.addClass("ntfy-sync-settings");
  ...
  private configureRuleSetting(setting: Setting, rule: RuleV1, index: number): void {
    ...
    setting.setDesc(info.description).setClass("ntfy-sync-rule-card");
...
function authForKind(kind: string): AuthConfig {
  return kind === "basic"
    ? { kind: "basic", username: "", password: "" }
    : kind === "bearer"
      ? { kind: "bearer", token: "" }
