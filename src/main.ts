...
import { Notice, Plugin, setIcon, setTooltip, type App } from "obsidian";
...
export default class NtfySyncPlugin extends Plugin {
  ...
  override async onload(): Promise<void> {
    ...
    try {
      ...
      if (!vaultBase) throw new Error("Desktop filesystem adapter is required");
      const pluginDirectory = join(
        vaultBase,
        this.manifest.dir ?? `.obsidian/plugins/${this.manifest.id}`,
      );
      this.stateStore = new JsonStateStore(pluginDirectory);
  ...
  private structureStatusTooltip(tooltip: HTMLElement): void {
    ...
    title.className = "ntfy-sync-status-tooltip-title";
    title.textContent = titleText;
    tooltip.insertBefore(title, arrow);

    const grid = document.createElement("div");
    grid.className = "ntfy-sync-status-tooltip-grid";
    for (const line of lines) {
      const separator = line.indexOf(": ");
      const row = document.createElement("div");
      row.className = "ntfy-sync-status-tooltip-row";
      const label = document.createElement("span");
      label.className = "ntfy-sync-status-tooltip-label";
      const value = document.createElement("span");
      value.className = "ntfy-sync-status-tooltip-value";
      label.textContent = separator >= 0 ? line.slice(0, separator) : line;
      value.textContent = separator >= 0 ? line.slice(separator + 2) : "";
