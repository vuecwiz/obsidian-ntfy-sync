...
export class NtfyConnectionRunner {
  ...
  private async runStream(signal: AbortSignal): Promise<void> {
    ...
    // requestUrl buffers the full response and cannot expose or abort an infinite NDJSON stream.
    const response = await window.fetch(this.subscriptionUrl(false), {
  ...
  private async runPoll(signal: AbortSignal): Promise<void> {
    ...
    while (!signal.aborted) {
      const response = await requestUrl({
        url: this.subscriptionUrl(true),
        headers: headersRecord(authHeaders(this.config.readAuth)),
        throw: false,
      });
      if (signal.aborted) return;
      if (response.status < 200 || response.status >= 300) throw responseFault(response);
      const parser = new NdjsonParser();
      await this.dispatch(parser.push(new TextEncoder().encode(response.text)));
      await this.dispatch(parser.finish());
